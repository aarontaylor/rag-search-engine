...
def answer_question(search_results, question, limit=5):
    ...
    # Use ALL of the retrieved results, not just [:limit]
    for i, result in enumerate(search_results, start=1):
        context += f"[{i}]: {result['title']}; {result['document']}\n\n"

    prompt = f"""You are a helpful movie expert answering questions for users of Hoopla, a movie streaming service.

Use both the provided movie descriptions and your own general knowledge about movies to answer the question. If the descriptions don't contain enough detail to fully answer, rely on your background knowledge to give the best possible answer.
...
- If the question is about characters, list the main characters by name
...
def question_command(question, limit=5):
    ...
    # Ask for plenty of candidates, same pattern as the other commands
    search_results = hybrid_search.rrf_search(
    ...
    # Let the LLM see (at least) those top results
