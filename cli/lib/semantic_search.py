...
def semantic_chunk(
    ...
    # 3. Handle single-sentence, no-punctuation case
    if len(sentences) == 1:
        s = sentences[0]
    ...
    # 5. Chunk into sentence groups with overlap
    chunks = []
    i = 0
    n_sentences = len(cleaned_sentences)

    while i < n_sentences:
        chunk_sentences = cleaned_sentences[i : i + max_chunk_size]

        # avoid tiny trailing chunk (matches Boot.dev behavior)
...
class ChunkedSemanticSearch(SemanticSearch):
    ...
    # ---------------------------------------
    # NEW METHOD: Chunked Semantic Search
    # ---------------------------------------
    def search_chunks(self, query: str, limit: int = 10) -> list[dict]:
        ...
        for cs in chunk_scores:
            m_idx = cs["movie_idx"]
            if m_idx not in movie_scores or cs["score"] > movie_scores[m_idx]["score"]:
                movie_scores[m_idx] = cs

        # sort movies by best score
        ranked = sorted(
            movie_scores.items(), key=lambda x: x[1]["score"], reverse=True
        )
        ...
        for _, info in ranked:
            doc = self.documents[info["movie_idx"]]
            title = doc["title"]
            ...
            result = format_search_result(
                ...
                document=desc[:100],
                score=score,
                chunk_info={"chunk_idx": info["chunk_idx"]},
            )
